...
function StrokeRegularPolygon(pos, corners, size, style, lineWidth) {
  ...
  ctx.moveTo(
    pos.x + size * Math.sin(Math.PI),
    pos.y + size * Math.cos(Math.PI),
  );
  for (let i = 0; i < corners; i++) {
    ctx.lineTo(
      pos.x + Math.sin(Math.PI - i * a) * size,
      pos.y + Math.cos(Math.PI - i * a) * size,
    );
  }
  ctx.lineTo(
    pos.x + size * Math.sin(Math.PI),
  ...
  ctx.lineTo(
    pos.x + size * Math.sin(Math.PI - 1 * a),
    pos.y + size * Math.cos(Math.PI - 1 * a),
  );
